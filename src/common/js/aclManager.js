...
export const ACL_POLICY_NFT_ALL = 'P1000';
export const ACL_POLICY_NFT_MINT = 'P1001'; // NFT 민팅
export const ACL_POLICY_NFT_BURN = 'P1002'; // NFT 소각

export const ACLKeyLabelMap = [
  // 계정
  { k: ACL_POLOCIY_GROUPCODE_ACCOUNT, l: '계정 관리' },
  ...
  // 유저
  { k: ACL_POLOCIY_GROUPCODE_USER, l: '유저 관리' },
  ...
  // 공지사항
  { k: ACL_POLOCIY_GROUPCODE_NOTIFICATION, l: '공지사항 관리' },
  ...
  // 우편함
  { k: ACL_POLOCIY_GROUPCODE_MESSAGE_INBOX, l: '우편함 관리' },
  ...
  // 푸쉬알림
  { k: ACL_POLOCIY_GROUPCODE_MESSAGE_PUSH, l: '푸쉬알림 관리' },
  ...
  // 이벤트
  { k: ACL_POLOCIY_GROUPCODE_EVENT_LOGINREWARD, l: '이벤트 관리' },
  ...
  // 활동로그/시스템로그
  { k: ACL_POLOCIY_GROUPCODE_ACTIVITYLOG, l: '로그 관리' },
  { k: ACL_POLICY_ACTIVITYLOG_VIEWLIST, l: '계정활동 로그 보기' },
  { k: ACL_POLICY_SYSTEMLOG_VIEWLIST, l: '시스템 로그 보기' },
  // 환경설정
  { k: ACL_POLOCIY_GROUPCODE_SETTING, l: '환경설정 관리' },
  { k: ACL_POLICY_SETTING_VIEW, l: '환경설정 보기' },
  { k: ACL_POLICY_SETTING_UPDATE, l: '환경설정 수정' },
  // NFT
  { k: ACL_POLOCIY_GROUPCODE_NFT, l: 'NFT 관리' },
  { k: ACL_POLICY_NFT_MINT, l: 'NFT 민팅' },
  { k: ACL_POLICY_NFT_BURN, l: 'NFT 소각' },
];

export const getLabelForKey = (aclKey) => {
  for (let item of ACLKeyLabelMap) {
    if (item.k === aclKey) {
      return item.l;
    }
  }
...
export const createBaseACLObj = () => {
  ...
  wholeACL[ACL_POLOCIY_GROUPCODE_NFT].push({ k: ACL_POLICY_NFT_MINT, v: 0 });
  wholeACL[ACL_POLOCIY_GROUPCODE_NFT].push({ k: ACL_POLICY_NFT_BURN, v: 0 });

  return wholeACL;
};

const groupCodeTable = [
...
export const createAdjustedACLObj = (aclInfo) => {

  const adjACLInfo = {...aclInfo};

  const baseACLInfo = createBaseACLObj();
  const aclKeySet = Object.keys(aclInfo);
  const baseACLKeySet = Object.keys(baseACLInfo);

  for (let baseACLKey of baseACLKeySet) {

    if(aclInfo.hasOwnProperty(baseACLKey) === false) {
      adjACLInfo[baseACLKey] = baseACLInfo[baseACLKey];
    } else {
      const aclUnitList = aclInfo[baseACLKey];
      const baseACLUnitList = baseACLInfo[baseACLKey];
      let found = false;
      for(let baseACLUnit of baseACLUnitList) {
        for(let aclUnit of aclUnitList) {
          if(aclUnit.k === baseACLUnit.k) {
            found = true;
            break;
          }
        }
        if(found === true) {
          ...
          adjACLInfo[baseACLKey] = [...aclInfo[baseACLKey],baseACLUnit];
        }
      }
    }
  }
...
export const makeACLInfoTable = () => {
  const aclInfoTable = [];
  const baseACL = createBaseACLObj();
  const aclKeySet = Object.keys(baseACL);
  let aclGroupObj;
  let aclItemList;
  for (let aclKey of aclKeySet) {
    aclGroupObj = {
      title: getLabelForKey(aclKey),
      aclUnitList: [],
    };
    aclItemList = baseACL[aclKey];
    for (let i = 1; i <= aclItemList.length; i++) {
      aclGroupObj.aclUnitList.push({ id: i, name: getLabelForKey(aclItemList[i - 1].k) });
    }
    aclInfoTable.push(aclGroupObj);
  }

  return aclInfoTable;
};

export const getACLKey = (groupIndex, keyIndex) => {
  const acl = createBaseACLObj();
  const groupKeyList = Object.keys(acl);
  const groupKey = groupKeyList[groupIndex];

  const aclUnitList = acl[groupKey];
  if (keyIndex >= aclUnitList.length) {
    return null;
  }
  return aclUnitList[keyIndex].k;
};

export const getGroupCode = (aclPolicyKey) => {
  const prefix = aclPolicyKey.substring(0, 3);
  const result = groupCodeTable.filter((e) => e.k === prefix);
...
export const isValidGroupCode = (groupCode) => {
  const result = groupCodeTable.filter((e) => e.k === groupCode);
  if (result === null || result.length === 0) {
    return false;
  } else {
    return true;
...
export const updateACL = (baseACL, aclToUpdate) => {
  let newACL = { ...baseACL };
  let groupACL;
...
export const updateACLUnitValue = (acl, unitKey, unitValue) => {
  ...
  if(aclGroup === null || aclGroup === undefined) {
    return acl;
  }
  //console.log('groupCode=',groupCode,',aclGroup=',aclGroup,',acl=',acl);
...
export const getACLGroup = (acl, group) => {
  return acl[group];
};

export const getACLUnitValue = (acl, unitKey) => {
  ...
  for (let aclUnit of aclGroup) {
    //console.log('aclUnit=', aclUnit, ',unitKey=', unitKey);
    if (aclUnit.k === unitKey) {
      return aclUnit.v;
    }
  }
...
export const checkAccessibleWithACL = (aclJsonStr, aclPolicyKey) => {
  //console.log('aclJsonStr=',aclJsonStr,',key=',aclPolicyKey);

  if (aclPolicyKey === null || aclPolicyKey === undefined || aclPolicyKey.trim() === '') {
    return false;
  }

  if (aclPolicyKey === ACL_POLICY_NONE) {
    return true;
  }
...
// ACL문자열이 유효한지 체크
export const isValidaACLFormat = (aclStr) => {
