...
const Table = (props) => {
    ...
    useEffect(() => {
        ...
        if(props.onResetTable !== undefined && props.onResetTable() === true) {
            resetPage();
        }
    ...
    return (
        ...
        <contentStyled.MainContentHeaderHorizontalLine />
        {
            props.bodyInfo.map((rowTable,idx1)=> {
                return <>
                    {rowTable.length>0 && <StTableBody {...props}>
                    {
                        rowTable.map((col,idx2)=>{
                            if(col!==null&&col.indexOf('__button') >= 0) {
                                const arr = col.split('=');
                            ...
                            } else if(col!==null&&col.indexOf('__checkbox') >= 0) {
