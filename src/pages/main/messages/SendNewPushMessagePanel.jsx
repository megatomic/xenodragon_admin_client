...
const SendNewPushMessagePanel = (props) => {
  ...
  const [sendType, setSendType] = useState(
    props.editMode
      ? props.editInfo.msgInfo.targetUserIDTable != null &&
        props.editInfo.msgInfo.targetUserIDTable.length > 0
        ? constants.MSGTARGET_TYPE_USER
        : constants.MSGTARGET_TYPE_ALL
      : constants.MSGTARGET_TYPE_ALL
  );
  const [targetUserIDTable, setTargetUserIDTable] = useState(
    props.editMode ? props.editInfo.msgInfo.targetUserIDTable : []
  );
  const [targetUserIDListStr, setTargetUserIDListStr] = useState('');
  ...
  const onTargetUserItemClick = useCallback((item) => {
    ...
    if (item.id !== constants.MSGTARGET_TYPE_USER) {
      setTargetUserIDTable([]);
      setTargetUserIDListStr('');
    }
  });
  ...
  const onTargetUserIDListChanged = (e) => {
    const table1 = e.target.value.trim().split(',');
  ...
  const onSendButtonClick = async (e) => {
    ...
    if (sendType === constants.MSGTARGET_TYPE_USER) {
      contentInfo.push(`타겟: 특정유저(${targetUserIDListStr})`);
    ...
    if (buttonIdx === 0) {
      ...
      if (props.editMode === true) {
        resultInfo = await requestModifyMessage({
          ...
          targetUserIDTable:
            sendType !== constants.MSGTARGET_TYPE_USER ? '' : targetUserIDTable,
      ...
      if (resultInfo.resultCode !== 0) {
        ...
        if (
          ...
          toast.info(
            `푸쉬메세지 전송이 예약되었습니다.(시간:${resultInfo.data.startTime})`,{autoClose:5000}
          );
          onCancelButtonClick(null);
  ...
  const sendingFinishCallback = (resultInfo) => {
    ...
    if (resultInfo.resultCode === 0) {
      toast.info(`푸쉬메세지 전송을 완료하였습니다(전송갯수:${resultInfo.data.totalSent},실패갯수:${resultInfo.data.failCount})`,{autoClose:5000});
  ...
  const onLoadCSVFileInfo = (e) => {
    const csvFileToArray = (string) => {
      ...
      console.log(`table=${JSON.stringify(table1, null, 2)}`);

      setTargetUserIDTable(table1);
      setTargetUserIDListStr(addressListStr);
    };
      ...
      <contentStyled.ContentBody>
        ...
        <contentStyled.FilterGroup marginLeft="5.4vw">
          <contentStyled.FilterItem>
            ...
            <span id="dropdown" style={{ marginLeft: '2vw' }}>
              <DropBox
                width="10vw"
        ...
        {sendType === constants.MSGTARGET_TYPE_USER ? (
          <styled.InputArea leftMargin="4vw" width="70%">
            ...
            <span className="row2">
              <TextArea1
                responsive="1.6"
                value={targetUserIDListStr}
                width="28vw"
                height="6vw"
                onChange={(e) => onTargetUserIDListChanged(e)}
              />
              <form>
                <input
                  type={'file'}
