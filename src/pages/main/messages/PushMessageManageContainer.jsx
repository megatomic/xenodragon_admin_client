...
const menuTable = [
  { id: 1, name: '푸쉬메세지 목록 조회' },
  { id: 2, name: '새 푸쉬메세지 보내기' },
  { id: 3, name: '이메일 보내기' },
];

const PushMessageManageContainer = () => {
  ...
  const onSubMenuChange = (menuID) => {
    ...
    if (menuID === 2 && aclManager.checkAccessibleWithACL(authInfo.accountInfo.aclInfo, aclManager.ACL_POLICY_MESSAGE_PUSH_SEND) === false) {
    ...
    <React.Fragment>
      ...
      {commonInfo.testMode ? (
        ...
        <mainStyled.MainContentPanel>
          <div>
            {activeMenuID === 1 && !msgEditMode && <QueryPushMessageListPanel onMsgEditModeChange={(flag, data) => onMsgEditModeChange(flag, data)} onSubMenuOpenClicked={(flag)=>onSubMenuOpenClick(flag)}/>}
...
export default PushMessageManageContainer;
