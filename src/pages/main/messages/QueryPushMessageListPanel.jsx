...
const tableHSpaceTable = '0.5fr 0.8fr 3fr 1fr 0.6fr 0.6fr';
...
const makeTableFromMsgList = (msgList) => {
  ...
  const result = msgList.map((msgInfo, idx) => {
    ...
    if (msgInfo.reservationFlag === true) {
      ...
      if (msgInfo.rewardData !== null && msgInfo.rewardData !== undefined) {
        const stateInfo = JSON.parse(msgInfo.rewardData);
        if (stateInfo.state === 1) {
          if (stateInfo.resultCode === 0) {
            sendResult = '성공';
          } else {
            sendResult = '실패';
          }
    ...
    let targetType = '전체';
    if (msgInfo.targetType.toUpperCase() === 'USER') {
      targetType = '특정 유저';
    } else if (msgInfo.targetType.toUpperCase() === 'ANDROID') {
      targetType = '안드로이드';
    } else {
      targetType = 'iOS';
    }
    return [
      '__checkbox',
      `${dayjs(msgInfo.creationTime).format('YYYY-MM-DD HH:mm')}`,
      '[' +
        getTitle(msgInfo.titleTable, getLangTypeFromCode(23)) +
        '] ' +
        getContent(msgInfo.contentTable, getLangTypeFromCode(23)),
      state,
      msgInfo.reservationFlag === false ? '완료' : sendResult,
      targetType,
    ];
  });
...
const QueryPushMessageListPanel = (props) => {
  ...
  const onSearchMessageClick = useCallback((event) => {
    ...
    if (startDate === '') {
      ...
      queryfilterInfo.filterStartTime =
        utils.makeDateTimeStringFromDate(startDate);
    }
  ...
  const onEditMsgButtonClick = (e) => {
    ...
    for (let mInfo of msgInfo.msgList) {
      ...
      if (mInfo.msgID === checkedMsgIDList[0]) {
        props.onMsgEditModeChange(true, {
  ...
  const onPopupButtonClick = async (buttonIdx) => {
    if (buttonIdx === 0) {
      ...
      const resultInfo = await requestDeleteMessages(
        constants.MESSAGE_TYPE_PUSHALARM,
        checkedMsgIDList
      );
  ...
  const reloadMessageList = async (queryFilterInfo) => {
    setStartLoading(true);
    setTimeout(async () => {
      const resultInfo = await requestMessageList({
        queryFilterInfo,
      ...
      <contentStyled.ContentHeader>
        ...
        <span id="button">
          <Button1
            ...
            width="6vw"
            height="2vw"
            onClick={(e) => onEditMsgButtonClick(e)}
          >
            보기/수정하기
          </Button1>
        </span>
        <span id="button">
          <Button1
            ...
            width="6vw"
            height="2vw"
            onClick={(e) => onDeleteMsgButtonClick(e)}
          >
            삭제
          </Button1>
        </span>
          ...
          <contentStyled.FilterItem>
            ...
            <span id="input">
              <InputField1
                ...
                readOnly={targetUserType !== constants.MSGTARGET_TYPE_USER ? true : false}
                value={targetUserID}
                onChange={(e) => onTargetUserIDChange(e)}
            ...
            <span id="search">
              <Button1
                ...
                width="4vw"
                height="1.8vw"
                onClick={(e) => onSearchMessageClick(e)}
              >
                검색
              </Button1>
            </span>
          ...
          <contentStyled.FilterItem>
            ...
            <span id="search">
              <Button1
                ...
                width="4vw"
                height="1.8vw"
                onClick={(e) => onSearchAllDurationUserClick(e)}
              >
                전체
              </Button1>
            </span>
