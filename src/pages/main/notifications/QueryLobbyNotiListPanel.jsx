...
const titleText = '로비 공지사항 조회';
const tableHeaderInfo = ['__checkbox', '생성 시각', '컨텐츠 타입', '제목/내용', '공지 현황', '작성자'];
const tableHSpaceTable = '0.5fr 1fr 0.8fr 2.5fr 0.8fr 0.8fr';
...
const makeTableFromNotificationList = (notificationList) => {
  ...
  const result = notificationList.map((notiInfo, idx) => {
    ...
    let dataType = '텍스트';
    if(notiInfo.dataType === constants.NOTIFICATION_DATATYPE_HTML) {
      dataType = 'HTML';
    } else if(notiInfo.dataType === constants.NOTIFICATION_DATATYPE_URL) {
      dataType = 'URL';
    }

    if(notiInfo.textContentType === false) {
      return ['__checkbox', dayjs(notiInfo.creationTime).format('YYYY-MM-DD HH:mm:ss'), dataType, getTitle(notiInfo.titleTable,getLangTypeFromCode(23)), state, notiInfo.creatorID];
    } else {
      return ['__checkbox', dayjs(notiInfo.creationTime).format('YYYY-MM-DD HH:mm:ss'), dataType, getTitle(notiInfo.titleTable,getLangTypeFromCode(23))+" / "+getContent(notiInfo.contentTable,getLangTypeFromCode(23)), state, notiInfo.creatorID];
    }
  });
...
const QueryLobbyNotiListPanel = (props) => {
  ...
  const reloadNotificationList = async (queryFilterInfo) => {
    ...
    setTimeout(async () => {
      const resultInfo = await requestNotificationList({ queryFilterInfo, notiType: constants.NOTIFICATION_TYPE_LOBBY, pageNo: 1 });
  ...
  const onPopupButtonClick = async (buttonIdx) => {
    if (buttonIdx === 0) {
      ...
      const resultInfo = await requestDeleteNotifications({notiIDList:checkedNotiIDList});
      console.log(resultInfo);
...
export default QueryLobbyNotiListPanel;
