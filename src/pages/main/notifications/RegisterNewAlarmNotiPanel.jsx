...
const titleText1 = '새 띠알림 공지사항 등록';
...
const RegisterNewAlarmNotiPanel = (props) => {
  ...
  const onRegisterButtonClick = async (e) => {
    ...
    if (props.editMode === true) {
      contentInfo.push("띠알림 공지사항을 업데이트하시겠습니까?");
    } else {
      contentInfo.push("새 띠알림 공지사항을 등록하시겠습니까?");
    }
    contentInfo.push(` `);
    ...
    if(buttonIdx === 0) {
      ...
      if (props.editMode === true) {
        resultInfo = await requestModifyNotification({
          ...
          notiType: constants.NOTIFICATION_TYPE_SCROLLALARM,
          dataType:0,
      ...
      if (resultInfo.resultCode !== 0) {
        ...
        if (props.editMode === true) {
          toast.info('띠알림 공지사항이 수정되었습니다.');
        } else {
          toast.info('새 띠알림 공지사항이 등록되었습니다.');
        }
      }
...
export default RegisterNewAlarmNotiPanel;
