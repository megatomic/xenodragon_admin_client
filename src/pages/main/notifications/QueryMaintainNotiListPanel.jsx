...
const makeTableFromNotificationList = (notificationList) => {
  ...
  const result = notificationList.map((notiInfo, idx) => {
    ...
    if (notiInfo.activationFlag === false) {
      state = '비활성화';
    } else if (notiInfo.scheduleNotiFlag === true) {
    ...
    return ['__checkbox', dayjs(notiInfo.creationTime).format('YYYY-MM-DD HH:mm:ss'), notiInfo.title, notiInfo.content, state, notiInfo.creatorID];
  });

  return result;
};

const QueryMaintainNotiListPanel = (props) => {
  ...
  const reloadNotificationList = async (queryFilterInfo) => {
    ...
    setTimeout(async () => {
      const resultInfo = await requestNotificationList({ queryFilterInfo, notiType: constants.NOTIFICATION_TYPE_MAINTENANCE, pageNo: 1 });
...
export default QueryMaintainNotiListPanel;
