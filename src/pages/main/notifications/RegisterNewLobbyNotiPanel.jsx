...
const RegisterNewLobbyNotiPanel = (props) => {
  ...
  const [dataType, setDataType] = useState(props.editMode ? props.editInfo.notiInfo.dataType : 0);
  ...
  const onDataTypeButtonClick = (idx) => {
  ...
  const onRegisterButtonClick = async (e) => {
    ...
    if (props.editMode === true) {
      contentInfo.push("로비 공지사항을 업데이트하시겠습니까?");
    } else {
      contentInfo.push("새 로비 공지사항을 등록하시겠습니까?");
    }
    contentInfo.push(` `);
    ...
    if(buttonIdx === 0) {
      ...
      if (props.editMode === true) {
        resultInfo = await requestModifyNotification({
          ...
          dataType,
          notiType: constants.NOTIFICATION_TYPE_LOBBY,
      ...
      if (resultInfo.resultCode !== 0) {
        ...
        if (props.editMode === true) {
          toast.info('로비 공지사항이 수정되었습니다.');
        } else {
          toast.info('새 로비 공지사항이 등록되었습니다.');
        }
      }
    ...
    <contentStyled.ContentWrapper>
      ...
      <contentStyled.ContentBody>
        ...
        <contentStyled.SettingGroupArea leftMargin="0vw" width="90%">
          <styled.OptionItem>
            <span id="option_title">컨텐츠 타입</span>
            <span id="col1">
              <RadioGroup responsive='1.6' initButtonIndex={dataType} interMargin="0.5vw" nameTable={['텍스트', 'HTML', '사이트 URL']} buttonClicked={(idx) => onDataTypeButtonClick(idx)} />
            </span>
          </styled.OptionItem>
          <br />
        ...
        <styled.InputArea leftMargin="4vw" width="70%">
          <span className="row1">
            <label>제목</label>
            <label>{dataType===constants.NOTIFICATION_DATATYPE_URL ?"사이트 URL":"내용"}</label>
          </span>
          <span className="row2">
            ...
            <TextArea1 responsive='1.8' value={getContent(contentTable,langType)} width="33vw" height={dataType===constants.NOTIFICATION_DATATYPE_URL?"2vw":"16vw"} placeholder={'내용을 입력하세요.(500자 이내)'} onChange={(e) => onContentChanged(e)} />
          </span>
          <span className="row3">&nbsp;</span>
        </styled.InputArea>
        <br />
...
export default RegisterNewLobbyNotiPanel;
