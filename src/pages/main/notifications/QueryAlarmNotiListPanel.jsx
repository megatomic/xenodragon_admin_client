...
const makeTableFromNotificationList = (notificationList) => {
  ...
  const result = notificationList.map((notiInfo, idx) => {
    ...
    let title = getTitle(notiInfo.titleTable,getLangTypeFromCode(23));
    if(title.trim() === '') {
      title = getTitle(notiInfo.titleTable,getLangTypeFromCode(10));
    }

    let content = getContent(notiInfo.contentTable,getLangTypeFromCode(23));
    if(content.trim() === '') {
      content = getContent(notiInfo.contentTable,getLangTypeFromCode(10));
    }

    return ['__checkbox', dayjs(notiInfo.creationTime).format('YYYY-MM-DD HH:mm:ss'), title, content, state, notiInfo.creatorID];
  });

  return result;
};

const QueryAlarmNotiListPanel = (props) => {
  ...
  const reloadNotificationList = async (queryFilterInfo) => {
    ...
    setTimeout(async () => {
      const resultInfo = await requestNotificationList({ queryFilterInfo, notiType: constants.NOTIFICATION_TYPE_SCROLLALARM, pageNo: 1 });
...
export default QueryAlarmNotiListPanel;
