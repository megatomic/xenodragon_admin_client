...
const titleText = '푸쉬알림 목록 조회';
const tableHeaderInfo = ['__checkbox', '생성 시각', '제목', '내용', '전송 상황', '대상'];
...
const makeTableFromMsgList = (msgList) => {
  ...
  const result = msgList.map((msgInfo, idx) => {
    ...
    return ['__checkbox', `${dayjs(msgInfo.creationTime).format('YYYY-MM-DD HH:mm')}`, getTitle(msgInfo.titleTable,getLangTypeFromCode(23)), getContent(msgInfo.contentTable,getLangTypeFromCode(23)), state, msgInfo.targetUserID === '' ? '전체' : msgInfo.targetUserID];
  });

  return result;
};

const QueryPushMessageListPanel = (props) => {
  const { startLoading, setStartLoading } = useCommon();
  const { msgInfo, requestMessageList, requestDeleteMessages } = useMessage();
  ...
  const onPopupButtonClick = async (buttonIdx) => {
    if (buttonIdx === 0) {
      ...
      const resultInfo = await requestDeleteMessages(constants.MESSAGE_TYPE_PUSHALARM, checkedMsgIDList);
  ...
  const reloadMessageList = async (queryFilterInfo) => {
    setStartLoading(true);
    setTimeout(async () => {
      const resultInfo = await requestMessageList({ queryFilterInfo, msgType: constants.MESSAGE_TYPE_PUSHALARM, pageNo: 1 });
  ...
  return (
    <contentStyled.ContentWrapper>
      ...
      <MediaQuery maxWidth={768}>
        ...
        <span>&nbsp;</span>
        <span id="button">
          <Button1 responsive='1.6' bgColor="var(--btn-confirm-color)" width="6vw" height="2vw" onClick={(e) => onEditMsgButtonClick(e)}>
            보기/수정하기
          </Button1>
        </span>
        <span id="button">
          <Button1 responsive='1.6' bgColor="var(--btn-secondary-color)" width="6vw" height="2vw" onClick={(e) => onDeleteMsgButtonClick(e)}>
            삭제
          </Button1>
        </span>
      </contentStyled.ContentHeader>
      ...
      <contentStyled.ContentBody>
        <contentStyled.FilterGroup>
          <contentStyled.FilterItem>
            ...
            <span id="dropdown">
              <DropBox responsive='1.3' 
                ...
                height="2vw"
                fontSize="0.6vw"
                text={filterTargetUserList[targetUserType - 1].name}
          ...
          <contentStyled.FilterItem>
            ...
            <span id="input">
              <InputField1 width="12vw" height="2vw" placeholder={'유저ID 또는 닉네임을 입력하세요.'} readOnly={targetUserType===1?true:false} value={targetUserID} onChange={(e) => onTargetUserIDChange(e)} onKeyPress={(e) => onKeyPress(e)} />
            </span>
            <span id="search">
              <Button1 responsive='1.6' bgColor="var(--btn-primary-color)" width="4vw" height="1.8vw" onClick={(e) => onSearchMessageClick(e)}>
                검색
              </Button1>
            </span>
          </contentStyled.FilterItem>
          <contentStyled.FilterItem>
            ...
            <span id="search">
              <Button1 responsive='1.6' bgColor="var(--btn-primary-color)" width="4vw" height="1.8vw" onClick={(e) => onSearchAllDurationUserClick(e)}>
                전체
              </Button1>
            </span>
          </contentStyled.FilterItem>
...
export default QueryPushMessageListPanel;
